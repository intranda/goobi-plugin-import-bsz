...
public class BSZ_BodenseeImport_Helper {
    ...
    /**
     * Read all years from the given JSON file to pass back an ordered list of all importable years for the mass import plugin GUI
     * 
    ...
    /**
     * Generates {@link ImportObject} element for given {@link Record} and store it in the list of {@link ImportObject} to be imported then afterwards
     * This method is the main entry to to the catalogue import of bibliographic data, the generation of a {@link Fileformat} and to enrich this with
     * more bibliographic data, to read the logical structure from the JSON file and assign pages with page labes to these structural elements
     * 
    ...
    /**
     * Method to do the catalogue request for a given identifier and to generate a {@link Fileformat} out of it During the creation of the
     * {@link Fileformat} it is enriched with an updated identifier, a viewer sub theme and the right digital collection assignements
     * 
    ...
    private void extractPdf(String inYear, String inProcessTitle, List<BSZ_BodenseeImport_Element> elements) throws IOException {
        ...
        for (Path entry : allMyFiles) {
            ...
            if (f.getAbsolutePath().contains("/aglv/") ||
                ...
                PDDocument inputDocument = Loader.loadPDF(f);
    ...
    /**
     * Method to add all issues into the the generated {@link Fileformat} incl. page labels, structural sub elements and page assignements. In case a
     * pdf file exists for a the volume too it is extracted into individual pages
     * 
    ...
    /**
     * Method to create or to adapt an identifier for a given structural element. The identifier that is named as parameter 'field' is searched in the
     * structural element and gets the prefix 'bsz' added if it is not there already. In case the identifier is not existing add it now to the
     * structural element incl. the prefix 'bsz'
     * 
